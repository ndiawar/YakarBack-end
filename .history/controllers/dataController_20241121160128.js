...
import { SerialPort } from 'serialport';  // Si vous utilisez un chemin d'importation ESM
import { ReadlineParser } from '@serialport/parser-readline';  // Correction ici
...
const port = new SerialPort({
  path,
  baudRate: 9600,
  autoOpen: false  // L'option autoOpen est définie sur false pour éviter que le port s'ouvre immédiatement
});

port.open((err) => {
  if (err) {
    console.error(`Erreur de connexion au port série ${path}:`, err);
  } else {
    console.log(`Connexion au port série ${path} réussie`);
  }
});

...
export const getDailyAverage = async (req, res) => {
  try {
    const averages = await Donnees.aggregate([
      {
        // Extraire uniquement la partie "jour" de la date (si "date" est de type Date)
        $addFields: {
          jour: { $dateToString: { format: "%Y-%m-%d", date: "$date" } }
        }
      },
      {
        // Grouper par la clé "jour"
        $group: {
          _id: "$jour", // La nouvelle clé contient uniquement la date
          avgTemperature: { $avg: "$temperature" },
          avgHumidite: { $avg: "$humidite" },
        },
      },
    ...
    console.error(err); // Log des erreurs pour les débogages
    res.status(500).json({

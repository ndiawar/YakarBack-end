...
export const getAllHistorique = async (req, res) => {
  try {
    const historiques = await Historique.aggregate([
      {
        $lookup: {
          from: 'users', // Nom de la collection des utilisateurs
          localField: 'id_user', // Clé dans la collection Historique
          foreignField: '_id', // Clé correspondante dans la collection User
          as: 'user', // Alias pour les données utilisateur
        },
      },
      {
        $unwind: { path: '$user', preserveNullAndEmptyArrays: true }, // Gère les utilisateurs inexistants
      },
      {
        $project: {
          _id: 1,
          ...
          heure: 1,
          id_user: 1,
          userName: { $ifNull: ['$user.name', 'Utilisateur inconnu'] }, // Nom ou "Utilisateur inconnu"
        },
      },
    ]);

...
export const updateUserRole = async (req, res) => {
  const userId = req.params.id;
  const { role } = req.body; // Le nouveau rôle transmis dans le corps de la requête

  try {
    if (!["admin", "user"].includes(role)) {
      return res.status(400).json({ message: 'Rôle invalide. Les rôles valides sont : "admin", "user".' });
    }
    ...
    user.roles = [role]; // Met à jour le rôle en remplaçant les existants
    ...
    res.status(500).json({ message: 'Erreur lors de la mise à jour du rôle utilisateur', error: err.message });
  }
};

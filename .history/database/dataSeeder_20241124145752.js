...
// Générer des données aléatoires pour chaque jour entre novembre 2024 et décembre 2024
...
// Générer des données pour une période allant de novembre 2024 à décembre 2024
const generateDataForDateRange = () => {
  const data = [];
  const startDate = new Date('2024-11-01'); // Commencer le 1er novembre 2024
  const endDate = new Date('2024-12-31'); // Fin le 31 décembre 2024

  let currentDate = new Date(startDate);

  // Nombre de données souhaitées (par exemple 500)
  const totalDataCount = 200;
...
const seedDatabase = async () => {
  try {
    ...
    console.log('200 données générées et insérées avec succès.');

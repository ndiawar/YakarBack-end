...
// Générer une date dans l'intervalle de janvier 2024 à février 2025
const generateDateInRange = (startDate, endDate) => {
  const randomDate = faker.date.between(startDate, endDate);
  return new Date(randomDate); // Retourne une date valide dans la plage
};
...
// Générer des données pour une période allant de janvier 2024 à février 2025
const generateDataForDateRange = () => {
  const data = [];
  const startDate = new Date('2024-01-01'); // Commencer à partir de janvier 2024
  const endDate = new Date('2025-02-01'); // Fin en février 2025
